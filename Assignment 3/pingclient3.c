...
void sendMessage(int sockfd, char* message, struct addrinfo* server) {
...
void recieveMessage(int sockfd, char* message, struct addrinfo* server) {
...
void checkMessage(uint32_t* sent, uint32_t recieved, float total_time) {
    if(*sent == recieved) {
    ...
    (*sent)++;//Increases sequence number
}
...
int main(int argc, char** argv) {
    ...
    fd_set read_set;
    uint32_t sequence_number = 0, rec = 0;
    int processing = 1;
    ...
    do {
        ...
        sendMessage(sockfd, (char* ) &sequence_number, server);
        ...
        if(sync_rv == 1) {
            ...
            recieveMessage(sockfd, (char* ) &rec, server);

            getCurrentTime(&end_time);
            float total_time = getTotalTime(start_time, end_time);

            checkMessage(&sequence_number, rec, total_time);
        }
        sleep(1);
